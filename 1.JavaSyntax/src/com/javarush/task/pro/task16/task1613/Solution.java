...
public class Solution {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
        ...
        DayOfWeek date = dateTime.getDayOfWeek();
        switch (date){
            case FRIDAY -> {if(dateTime.getHour()>=22 && dateTime.getMinute()>=00) return true;}
            case SATURDAY -> {return true;}
            case SUNDAY -> {if(dateTime.getHour()<23 ) return true;}
        }
        return false;
    }
}
